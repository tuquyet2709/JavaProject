...
public class Rule {
    public static boolean checkValid(Xau x) {
        ...
        if (checkPhuAmCuoiNgNh(x) == false) return false;
        return true;
    }
    ...
    private static boolean checkPhuAmCuoiNgNh(Xau x) {
        if (x.getPhu_am_dau().equals("")){
            if (x.getPhu_am_cuoi().equals("ng")){
                if ("ăắằẳẵặâấầẩẫậiìíỉĩịyỳýỷỹỵèéẻẽẹêềếểễệơờớởỡợ".contains(x.getNguyen_am())) return false;
            }
            if (x.getPhu_am_cuoi().equals("nh")){
                if ("ăắằẳẵặâấầẩẫậyỳýỷỹỵeèéẻẽẹêềếểệoòóỏõọôồốổỗộơờớởỡợ".contains(x.getNguyen_am())) return false;
            }
        }
        return true;
    }
